- [day01: 開発環境](#day01-開発環境)
  - [1.Python](#1python)
    - [1.1. python コマンドの種類](#11-python-コマンドの種類)
    - [1.2. python コマンドのインストール](#12-python-コマンドのインストール)
      - [mac の人向けのお話](#mac-の人向けのお話)
    - [1.3. インストールした `python.exe` は直接には使いません．](#13-インストールした-pythonexe-は直接には使いません)
  - [2. vscode](#2-vscode)
    - [2.1. インストール](#21-インストール)
    - [2.2. vscode の使い方](#22-vscode-の使い方)
    - [2.2. vscode の拡張機能](#22-vscode-の拡張機能)
  - [3. poetry](#3-poetry)
  ...
  - [4. **vscode でコーディング**](#4-vscode-でコーディング)
  - [5. jupyterhub の利用](#5-jupyterhub-の利用)
...
0. MS Python（ダミー）: Windows に最初から入っているもの
    > `C:\Users\アカウント名\AppData\Local\Microsoft\WindowsApps\python.exe`
1. MS python : Microsoft Store から入手したもの
    > `C:\Users\アカウント名\AppData\Local\Microsoft\WindowsApps\PythonSoftwareFoundation.Python(Version)\*\*\*\python.exe`
2. Anaconda python : Anaconda に付属してきたもの
    > `C:\Users\アカウント名\anaconda3\Scripts\python.exe`
3. 素 python : 公式サイト python.org から入手したもの
    > `C:\Users\アカウント名\AppData\Local\Programs\Python\Python(Version)\python.exe`

次のコマンドを コマンドプロンプト(Powershellではありません)で実行すると，いろいろ出てくると思います．

```
> dir /b /s %LOCALAPPDATA% | findstr /r /i "\python.exe$"
```
...
>
> - **これはアプリケーション本体ではありません．ただのショートカット（「アプリ実行エイリアス」）です．**
> - この`python`を実行すると，Microsoft Store の Python のページが立ち上がります．「入手」を押すと，1 の MS Python を入手することになります．
> - 「アプリ実行エイリアス」は予期しない動作となるので，機能しないようにしておくことを勧めます．
>    - 「設定」の「アプリと機能」に「アプリ実行エイリアス」というのがあり，その項目に，「アプリインストーラー」というものがあります．それを「オフ」にしておきましょう．
...
ぶっちゃけ何でも構いませんが，皆と同じ環境にしたいなら，演習 Ⅱ では「素 python」をおすすめします．すなわち[公式サイト](https://www.python.org/) のものをインストールする，ということになりますが，，，，

**おすすめのソフトウェアインストール方法** があります．`winget` を使う方法です．

`winget` は，Microsoft謹製のパッケージ管理ソフトです．パッケージ管理とはバージョン管理とも云われ，ここで管理対象になった（インストールした）ソフトウェアは，コマンド一発でアップグレードされます．

1. winget でインストールできる Python関連ソフトウェアを探す．
    ```
    ...
    :
    Python 3.14               Python.Python.3.14             3.14.4               winget
    Python 3.9                Python.Python.3.9              3.9.13               winget
    Python Install Manager    Python.PythonInstallManager    26.1.240.0           winget
    :
    winpython                 winpython.winpython            7.0.20231126final    winget
    :
    ````
2. IDが `Python.Python.X.Y` なのが公式Pythonで，最新は `Python.Python.3.14` のようです．
    これをインストールしてもよいのですが，公式には，インストールマネージャを利用することが勧められています．
    よって，まず `Python.PythonInstallManager` をインストールします．
    「名前」ではなく，「ID」を指定します．
    ````
    > winget remove --id Python.Launcher
    > winget install --id Python.PythonInstallManager
    ````
    * インストールされるコマンドは `py` です． 実は，少し前まで `py` は Python.Launcher のことだったので， Python.PythonInstallManager をインストールする前に Python.Launcherをアンインストールしています．
3. 最新のPythonをインストールします．
    ````
    > py install default
    ...
    Directory to add: C:\Users\アカウント名\AppData\Local\Python\bin
    ````

    最後に書いてあることは，
    - 「グローバル・ショートカットが収められたディレクトリ」が環境変数 `%PATH%` 上に無い．
    - グローバルなPythonのエイリアス（別称）を利用したければ，`%PATH%` にそのディレクトリを追加せよ．
    - 追加するディレクトリは，〜
    です．Pythonのエイリアスは必要ないので，何もしないことにします．
4. これでPython最新版が使えます．
    - どこに `python.exe` があるでしょうか？確認します．
        ```
        > dir /b /s %LOCALAPPDATA% | findstr /r /i "\python.exe$"

        C:\Users\アカウント名\AppData\Local\Python\bin\python.exe
        C:\Users\アカウント名\AppData\Local\Python\pythoncore-3.13-64\python.exe
        C:\Users\アカウント名\AppData\Local\Python\pythoncore-3.14-64\python.exe
        ```
    - コマンドプロンプトで `python.exe` だとか `python` と呼んでも起動しません．
        ```
        > python.exe
        ...
        ```
    - そもそもコマンドプロンプトで呼べるコマンドは `%PATH%` （環境変数 `PATH`） に列挙されたディレクトリの中のものです．ということで，手順3の最後にあった
        > PATH に `C:\Users\アカウント名\AppData\Local\Python\bin` を加えるといいよ．
        につながるわけですが，**`python` ではなく，`py` でPythonが使える** ので，そっちで行きましょう．
        ```
        > where python
        ...
        C:\Users\アカウント名\AppData\Local\Microsoft\WindowsApps\py.exe
        ```
...
5. ちなみに，`py` の使い方をいくつか．
    - Python を新しくしたければ，
        ...
        > py install --update
        ```
    - Python Install Manager で管理しているPython (`*` が付いているものが `py` で起動するPython)
        ```
        > py list
...
### 1.3. インストールした `python.exe` は直接には使いません．

インストールした `python.exe` は直接には使いません．

アプリを作ろうとなったときごとに，そのプロジェクト専用の「仮想環境」を用意します．その「仮想環境」の中に `python.exe` を入れて（実はインストール済みのコピー），それを使います．

同じものなのになぜわざわざ仮想環境のものを使うのか？
- 起動する `python.exe` は，ライブラリ（外部パッケージ）を使うことになりますが，Pythonは同じであっても，使用するライブラリ構成が異なります．
- `Dir1\Script\python.exe` を起動した場合は，ライブラリの場所が `Dir1\Lib\PythonX.Y\site-packages` を使うことが決められています．ライブラリ構成をアプリごとに使い分けるには，起動する `python.exe` の場所を別にしておかなければいけない，というわけです．
...
どうせなので，さきほどPythonインストールでも登場した `winget` を使います．

```
> winget search --id VisualStudioCode

名前                                         ID                                         バージョン    ソース
----------------------------------------------------------------------------------------------------------
Microsoft Visual Studio Code                 Microsoft.VisualStudioCode                 1.116.0       winget
Microsoft Visual Studio Code CLI             Microsoft.VisualStudioCode.CLI             1.116.0       winget
Microsoft Visual Studio Code Insiders        Microsoft.VisualStudioCode.Insiders        1.117.0       winget
Microsoft Visual Studio Code Insiders CLI    Microsoft.VisualStudioCode.Insiders.CLI    1.117.0       winget
```
...
```{.sh}
> poetry new --interactive kadai01
```
...
`kadai01`ディレクトリの中にさらに `src` というディレクトリ，さらにその下に `kadai01` というディレクトリができています．この深い位置の方の `kadai01` が配布されるパッケージです．このディレクトリの中にあなたがモジュールを作成します．すでに`__init__.py` というものがありますが，とりあえず今は気にせずそのままに．
