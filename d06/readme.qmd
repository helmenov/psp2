...
`mydata1.csv` と同じようにするには，まず，'名前' を `index` に使わないようにします．（ `.reset_index(['名前'])` ）

```{python}
df4 = df3.reset_index(['名前'])
df4
```
...
2. 尺度を設定
    - 質的変数（文字，数字）:
        - 名義尺度：`se = pd.Categorical(categories=[...], ordered=False)`
        - 順序尺度: `se = pd.Categorical(categories=[...], ordered=True)`
            - 場合によっては間隔尺度に設定しなおす．
    - 量的変数（数値）：
        - 間隔尺度・非尺度: `se = se.astype('float')`
...
また，このデータは所謂long形式ですので，`pivot`で wide形式になおしてから使ってください．
