...
配布パッケージ（フォルダ） `d02` の下の `data` というフォルダにリソースファイル `mydata1.csv` を置いているとします．
...
- [`mydata1.csv`]('https://raw.githubusercontent.com/helmenov/psp2/main/d06/k02/data/mydata1.csv')
- [`mydata2.csv`]('https://raw.githubusercontent.com/helmenov/psp2/main/d06/k02/data/mydata2.csv')
- [`mydata3.csv`]('https://raw.githubusercontent.com/helmenov/psp2/main/d06/k02/data/mydata3.csv')
- [`heights14.csv`]('https://raw.githubusercontent.com/helmenov/psp2/main/d06/k02/data/heights14.csv') : 課題用
...
1. `df[['var']]` : DataFrame 2階テンソル（行列）
2. `df['var']` : Series 1階テンソル（ベクトル）
3. `df.var` : Series 1階テンソル（ベクトル）
4. `df[['var']][:2]` : DataFrame
5. `df.loc[:1, 'var']` : Series
6. `df.iloc[:2, 2]` : Series
...
```
    ...
    尺度を設定する前のdtypeは，`object`．object型というのは，汎用型であり，所謂文字列型(str)と同義です．
...
セロリ好き度の5段階は等間隔ですので，*「数にしたからには間隔尺度になります」* が，そうなっていません．

間隔尺度にしてあげましょう．

`.cat.codes` は，カテゴリに振られた符号（数値）を参照します．
...
### 1.8.4. クロス集計表を作ることもできます．クロスさせる2つの変数名を引数に並べます．
...
[これ](heights14.csv)は，身近の14人（男7人，女7人）に性別と身長を尋ねたときの回答を集めた標本データである．`k02/k02/data/heights14.csv` とする．
