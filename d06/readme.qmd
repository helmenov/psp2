...
```{python}
# まず，wide形式にしたときのindex列を決めます．リストで指定します．
df2 = df.set_index(["ID"])
df2
```
...
```{python}
# index列が変わりました
# columnsをwide形式の変数名が並んでる列名，今回は "Q"
# valuesをwide形式の各変数の値が並んでる列名，今回は "A"
# としてピボット
df3 = df2.pivot(columns="Q", values="A")
display(df3)
print(df3.dtypes)
```

wide形式になりましたが，変数のtypeが`object`（なにがし）になっています．
...
```{python}
df3['性別'] = pd.Categorical(df3['性別'], categories={'男','女'}, ordered=False)
df3['身長'] = df3['身長'].astype(float)
display(df3)
print(df3.dtypes)
```
...
- 母集団の平均の最良推定値（「◯±△」の「◯」）は，標本平均($\bar{x}$)と等しい．
- また，標本誤差$\mathrm{SE}$（「◯±△」の「△」）は， $\sqrt{\frac{u^2}{N}}$ で求まる．
...
男性全体，女性全体の母集団の標準偏差 $u$ を求めよ．モジュール名は，`k02/k02_3.py`とする．
母集団の標準偏差は，`k02_2` の母集団分散 $u^2$ の二乗根である．
